...
export default function AdminDashboard() {
    ...
    // ── CV upload modal state (applications) ──────────────────────
    const [cvModal, setCvModal] = useState(null);       // application item
    ...
    const handleNewsImageUpload = (newsId, imageFile) => {
        dispatch(uploadNewsImage({ id: newsId, imageFile })).then((res) => {
            if (!res.error) refreshNews();
        });
    };

    // ── Application CV upload handler ─────────────────────────────
    const handleCvUpload = () => {
        if (!cvFile || !cvModal) return;
        dispatch(uploadApplicationCV({ id: cvModal.id, cvFile })).then((res) => {
            if (!res.error) {
                dispatch(fetchApplications({ branch: branchId }));
                setCvModal(null);
                setCvFile(null);
            }
        });
    ...
    return (
        <div className="admin-layout">
                        ...
                        <div className="admin-list-header">
                            ...
                            <div className="admin-filters">
                                {['All', ...new Set(categoriesList.map((c) => c.name))].map((c) => (
                                    <button key={c} onClick={() => setCatFilter(c)} className={`admin-filter-btn ${catFilter === c ? 'active' : ''}`}>
                                        {c === 'All' ? 'Barchasi' : c}
                                    </button>
                                ))}
                            </div>
                        ...
                        {newsLoading ? (
                            ...
                            <div className="admin-table-wrap">
                                <table className="admin-table">
                                    <thead>
                                    <tr>
                                        <th>Yangilik</th><th>Kategoriya</th><th>Sana</th><th>Holati</th><th>Harakatlar</th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                        ...
                                        <tr key={item.id}>
                                            <td>
                                                <div className="post-cell">
                                                    {/* ── Rasm ustiga bosish → upload ── */}
                                                    <div className="post-thumb" style={{ position: 'relative', cursor: 'pointer' }}
                                                         title="Rasmni yangilash uchun bosing"
                                                         onClick={() => {
                                                             ...
                                                             input.onchange = (e) => {
                                                                 ...
                                                                 if (file) handleNewsImageUpload(item.id, file);
                                                             };
                                                             input.click();
                                                         }}>
                        ...
                        onSave={(data) => {
                            if (view === 'news-create') {
                                dispatch(createNews({ ...data, branch: branchId })).then((res) => {
                                    if (!res.error) {
                                        // Agar rasm tanlangan bo'lsa alohida upload qilamiz
                                        if (data.image instanceof File && res.payload?.id) {
                                            handleNewsImageUpload(res.payload.id, data.image);
                                        }
                                        refreshNews();
                                        setView('news-list');
                                    }
                                ...
                                dispatch(updateNews({ id: editItem.id, ...data, branch: branchId })).then((res) => {
                                    if (!res.error) {
                                        if (data.image instanceof File) {
                                            handleNewsImageUpload(editItem.id, data.image);
                                        }
                                        refreshNews();
                                        setView('news-list');
                                    }
                        ...
                        <div className="admin-list-header">
                            ...
                            <div className="admin-filters">
                                {['All', ...TYPE_CHOICES].map((t) => (
                                    <button key={t} onClick={() => setTypeFilter(t)} className={`admin-filter-btn ${typeFilter === t ? 'active' : ''}`}>
                                        {t === 'All' ? 'Barchasi' : t}
                                    </button>
                                ))}
                            </div>
                        ...
                        {applicationsLoading ? (
                            ...
                            <div className="admin-table-wrap">
                                <table className="admin-table">
                                    <thead>
                                    <tr>
                                        <th>Ism / Vakansiya</th><th>Aloqa</th><th>Holati</th><th>Sana</th><th>CV (Rezyume)</th><th>Harakatlar</th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                    ...
                                    {filteredApplications.map((item) => (
                                        <tr key={item.id}>
                                            ...
                                            <td className="date-cell">{item.created_at?.slice(0, 10) || '—'}</td>
                                            <td>
                                                {/* ── CV ko'rish + yangi CV yuklash ── */}
                                                <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
                                                    {item.cv_file ? (
                                                        <a href={item.cv_file} target="_blank" rel="noreferrer"
                                                           className="news-cat-badge"
                                                           style={{ backgroundColor: 'var(--navy)', display: 'inline-block', padding: '6px 12px', fontSize: 11, cursor: 'pointer', textDecoration: 'none' }}>
                                                            Rezyumeni ko'rish
                                                        </a>
                                                    ) : (
                                                    ...
                                                    <button
                                                        ...
                                                        title="CV yuklash"
                                                        onClick={() => { setCvModal(item); setCvFile(null); }}>
                                                        <Upload size={14} />
                                                    </button>
                                                </div>
                                            </td>
            ...
            {/* ═══ CV Upload Modal ═══ */}
            {cvModal && (
                <div className="modal-overlay" onClick={() => { setCvModal(null); setCvFile(null); }}>
                    <div className="delete-modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: 400 }}>
                        <h3 style={{ marginBottom: 4 }}>CV yuklash</h3>
                        <p style={{ marginBottom: 20 }}>{cvModal.name} — {cvModal.phone}</p>

                        <div style={{ display: 'flex', flexDirection: 'column', gap: 14, textAlign: 'left' }}>
                            {/* File input zone */}
                            <div
                                style={{
                                    border: '2px dashed #c7d2fe', borderRadius: 12, padding: '24px 16px',
                                ...
                                <p style={{ fontSize: 14, color: '#374151', margin: 0 }}>
                                    {cvFile ? cvFile.name : 'CV faylini tanlash uchun bosing'}
                                </p>
                                <span style={{ fontSize: 12, color: '#94a3b8' }}>PDF, DOC, DOCX</span>
                            </div>
                            ...
                            {cvModal.cv_file && (
                                <div style={{ fontSize: 13, color: '#64748b' }}>
                                    Mavjud CV:{' '}
                                    <a href={cvModal.cv_file} target="_blank" rel="noreferrer"
                            ...
                            <div className="delete-actions" style={{ marginBottom: 0 }}>
                                <button className="btn btn-outline" onClick={() => { setCvModal(null); setCvFile(null); }}>
                                    Bekor qilish
                                </button>
                                <button className="btn btn-primary" disabled={!cvFile || applicationsLoading} onClick={handleCvUpload}>
                                    {applicationsLoading ? <span className="al-spinner" /> : <Upload size={14} />}
                                    {' '}{applicationsLoading ? 'Yuklanmoqda…' : 'Yuklash'}
                                </button>
                            </div>
                        </div>
                    </div>
                ...
                <div className="publish-toggle">
                    <label className="publish-label">
                        <input type="checkbox" checked={form.published}
                               onChange={(e) => setForm({ ...form, published: e.target.checked })} className="publish-check" />
                ...
                <div className="publish-toggle">
                    <label className="publish-label">
                        <input type="checkbox" checked={form.is_active}
                               onChange={(e) => setForm({ ...form, is_active: e.target.checked })} className="publish-check" />
                        <span className="publish-track"><span className="publish-thumb" /></span>
